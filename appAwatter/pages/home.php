<?php
session_start();
$servidor = "92.205.11.133:3306";
$usuario = "desqu_agrocota";
$senha_db = "Douglas@2019";
$dbname = "desquinela_sqlserver";

$email = $_SESSION['usuario'];

$link = mysqli_connect($servidor, $usuario, $senha_db, $dbname);
mysqli_set_charset($link, "utf8");

$sql =  "SELECT `nome` FROM `Usuarios` WHERE `email` = '$email' limit 1";
$query = mysqli_query($link, $sql);
?>

<!doctype html>
<html lang="en">

<head>
  <!-- Required meta tags -->
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <!-- Bootstrap CSS -->
  <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.0.2/dist/css/bootstrap.min.css" rel="stylesheet">
  <link href="../style/home.css" rel="stylesheet">
  <title>Awatter</title>
</head>

<body>
  <div class="container">
    <div class="fundo">
      <div class="data">
        <p id="data"></p>
      </div>
      <div class="pesquisa">
        <div class="pesquisa-input">
          <input type="text" class="input-form" aria-label="Username">
          <a><img class="lupa" src="data:image/png;base64,"></a>
        </div>
      </div>
      <div class="user">
        <div class="text-center">
          <img src="../img/senhor.jpeg" class="circulo" alt="...">
        </div>
        <p class="login-nome"> Olá !!!<br>
          <?php while ($row = mysqli_fetch_assoc($query)) {
            echo $row['nome'];
          }
          ?>
        </p>
      </div>

      <div class="btn-progress">
        <button type="submit" class="btn">Visão geral</button>
        <a href="../pages/produtividade.php" class="link">Produtividade</a>

        <div class="banner">

          <h1 class="label1">Vamos Trabalhar!!!</h1>
          <h3 class="label1">Veja aqui o progresso da sua horta !</h3>
          <div class="barra">
            <div class="progress">
              <div class="progress-bar progress-bar-striped active" role="progressbar" aria-valuenow="45" aria-valuemin="0" aria-valuemax="100" style="width: 45%">
              </div>
            </div>
          </div>
        </div>
      </div>
  <div class="btn-calendario">    
      <p class="calendario">Calendário</p>
      <div class="bloco1">
        <p class="gastos">Gastos</p>
        <img src="../img/icone4.png" width="50px" class="icone1" alt="...">
        <div class="barra-progress">
          <div class="progress">
            <div class="progress-bar progress-bar-striped active" role="progressbar" aria-valuenow="45" aria-valuemin="0" aria-valuemax="100" style="width: 45%">
            </div>
          </div>
        </div>
      </div>

          <div class="bloco2">
        <p class="gastos">Baixos Níveis</p>
        <img src="../img/icone3.png" width="50px" class="icone1" alt="...">
        <div class="barra-progress">
          <div class="progress">
            <div class="progress-bar progress-bar-striped active" role="progressbar" aria-valuenow="45" aria-valuemin="0" aria-valuemax="100" style="width: 45%">
            </div>
          </div>
        </div>
      </div>

       

      <div class="bloco3">
        <p class="gastos">Baixos Níveis</p>
        <img src="../img/icone2.png" width="50px" class="icone1" alt="...">
        <div class="barra-progress">
          <div class="progress">
            <div class="progress-bar progress-bar-striped active" role="progressbar" aria-valuenow="45" aria-valuemin="0" aria-valuemax="100" style="width: 45%">
            </div>
          </div>
        </div>
      </div>

      
      <div class="bloco4">
        <p class="gastos">Altos Níveis</p>
        <img src="../img/icone1.png" width="50px" class="icone1" alt="...">
        <div class="barra-progress">
          <div class="progress">
            <div class="progress-bar progress-bar-striped active" role="progressbar" aria-valuenow="45" aria-valuemin="0" aria-valuemax="100" style="width: 45%">
            </div>
          </div>
        </div>
      </div>
</div>


      <div class="footer">Desenvolvido por Douglas Esquinelato®</div>
    </div>
    <script>
      var semana = ["Domingo", "Segunda-Feira", "Terça-Feira", "Quarta-Feira", "Quinta-Feira", "Sexta-Feira", "Sábado"];
      var d = new Date();
      document.getElementById("data").innerHTML = semana[d.getDay()];
    </script>
</body>

</html>